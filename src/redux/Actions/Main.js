...
const getAllData = async (path)=>{
    const alldata =[]
   await RNFS.readDir(path) // On Android, use "RNFS.DocumentDirectoryPath" (MainBundlePath is not defined)
    .then(async (result) => {
 ...
 try {
    await getAllData(RNFS.ExternalStorageDirectoryPath  +  "/Documents").then(res=>{
      dispatch({
